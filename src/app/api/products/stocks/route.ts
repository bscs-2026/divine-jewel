...
export async function GET() {
    try {
        ...
        const rows = await query('SELECT * FROM stocks_record');
...
export async function POST(request: NextRequest) { 
    ...
    try {
        ...
        const result = await query(
            'INSERT INTO `stocks_record` (inventory_id, status, date, quantity) VALUES (?, ?, ?, ?)',
            [inventory_id, status, date, quantity]
        );
        console.log('Added stocks_record:', result);
