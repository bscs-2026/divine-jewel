...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        console.log('Updating supply_data in database...');
        ...
        const result = await query(
            ...
            [supplier_id, sku, material_name, quantity, unit_of_measure, price_per_unit, destination_branch_id || null, employee_id || null, note || null, status, id]
        );
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        console.log(`Deleting supply_data with id ${id} from database...`);

        const result = await query(
            'DELETE FROM `supply_data` WHERE id = ?',
            [id]
        );
