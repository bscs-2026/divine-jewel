...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
    try {
        const { id } = params;
        const { branch_code, quantity, employee, batch_id} = await request.json();
        ...
        await query(
            'INSERT INTO `stock_details` (batch_id, action, product_id, destination_branch, quantity, employee_id) VALUES (?, ?, ?, ?, ?, ?)',
            [batch_id, 'Add', id, branch_code, quantity, employee || 20]
        );
